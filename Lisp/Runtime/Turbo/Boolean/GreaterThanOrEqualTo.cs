...
public class GreaterThanOrEqualTo : ITurboFunction
{
    ...
    public BaseLispValue Execute(Node function, List<Node> arguments, LispScope scope)
    {
        ...
        if (right is not LispNumberValue rightNumber) throw Report.Error(new WrongArgumentTypeReportMessage("Expected the right value to be a number"), arguments[0].Location);
