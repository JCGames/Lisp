...
public class GreaterThanOrEqualTo : ITurboFunction
{
    ...
    public BaseLispValue Execute(Node function, List<Node> parameters, LispScope scope)
    {
        ...
        if (right is not LispNumberValue rightNumber) throw Report.Error(new WrongArgumentTypeReportMessage("Expected the right value to be a number"), parameters[0].Location);
